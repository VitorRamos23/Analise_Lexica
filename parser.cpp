...
Parser::statement()
{
    ...
    {
        // Esta lógica agora funciona porque peekToken() foi implementado no scanner
...
{
    match(READ);
    // <--- CORRIGIDO: Removidos parênteses para seguir a gramática 'read LValue' --->
...
Parser::lValueComp()
{
    if (lToken->name == SEP_DOT)
    {
        ...
        */
        ...
        // <--- CORRIGIDO: 'else if' mudado para 'if' para tratar 'obj.array[0]' --->
        // Se o bloco acima for removido, este deve ser um 'if' simples.
